...
/* -- titles and digits ---------------------------------------------- */
enum { TT_BIG,TT_SUPER,TT_MEGA,TT_EPIC,
       TT_JULT,TT_JMEGA,TT_JMAJOR,TT_JMINOR,      /* in JP_* order */
       TT_JACKPOT,TT_X2,TT_X3,TT_X4,TT_X5,TT_MULTUP,TT_BADGE,NTT };
static fxspr_t titleS[NTT];
...
static void fx_init(void){
  ...
  fxHalo=0xE08A10;
  fx_bake_text(&titleS[TT_BIG],  "BIG WIN",  16,GOLDG,5,14);
  fxHalo=0xE0207A;
  fx_bake_text(&titleS[TT_SUPER],"SUPER WIN",16,FX_SUPERG,5,14);
  fxHalo=0x1E78E0;
  fx_bake_text(&titleS[TT_MEGA], "MEGA WIN", 16,FX_MEGAG,5,14);
  fxHalo=0x9A28E0;
  fx_bake_text(&titleS[TT_EPIC], "EPIC WIN", 16,FX_EPICG,6,16);
  fxHalo=0x40B0FF;
  fx_bake_text(&titleS[TT_JULT],  JP_NAME[JP_ULT],  19,RAINBOWG,6,16);
  fxHalo=0xE03010;
  fx_bake_text(&titleS[TT_JMEGA], JP_NAME[JP_MEGA], 19,FX_FIREG,5,16);
  fxHalo=0x8A30E0;
  fx_bake_text(&titleS[TT_JMAJOR],JP_NAME[JP_MAJOR],19,FX_MAJORG,5,16);
  fxHalo=0x10B090;
  fx_bake_text(&titleS[TT_JMINOR],JP_NAME[JP_MINOR],19,FX_MINORG,5,16);
  fxHalo=0xD08010;
  fx_bake_text(&titleS[TT_JACKPOT],"JACKPOT",9,GOLDG,5,8);
  fx_bake_text(&titleS[TT_X2],"X2",16,GOLDG,5,12);
  fx_bake_text(&titleS[TT_X3],"X3",16,GOLDG,5,12);
  fx_bake_text(&titleS[TT_X4],"X4",16,GOLDG,5,12);
  fx_bake_text(&titleS[TT_X5],"X5",16,GOLDG,5,12);
  fx_bake_text(&titleS[TT_MULTUP],"MULTIPLIER UP",5,SILVERG,4,6);
  fxHalo=0;
  fx_bake_text(&titleS[TT_BADGE],"X2",3,GOLDG,5,0);
...
static void fx_deinit(void){
  ...
  for(int i=0;i<NTT;i++) fxs_free(&titleS[i]);
...
static void fx_light_cluster(uint32_t m,uint32_t c,float pulse,int heavy,float popT){
  ...
  for(int i=0;i<NCELL;i++){
    ...
    const fxspr_t*t=&titleS[TT_BADGE];
...
static void fx_bigwin_draw(void){
  ...
  /* title */
  const fxspr_t*s=&titleS[TT_BIG+T-1];
  const fxspr_t*old=T>1?&titleS[TT_BIG+T-2]:NULL;
...
static void fx_jackpot_draw(void){
  ...
  const fxspr_t*s=&titleS[TT_JULT+tier];
  ...
  if(jt>0){
    ...
    const fxspr_t*j=&titleS[TT_JACKPOT];
...
static void fx_multup_draw(void){
  ...
  const fxspr_t*ttl=&titleS[TT_MULTUP];
  ...
  const fxspr_t*s=&titleS[TT_X2+m-2];
