...
every pixel and every sound generated by the code itself.
...
- **Hollywood presentation**: BIG / SUPER / MEGA / EPIC win count-ups,
  1,000-particle 3-D coin showers, lightning, fireworks, a live marquee
...
| **Attract loop** - the machine shows itself off between players | **Free spins** - every wild reel adds +1 to the multiplier, and it never goes down until the feature ends |
...
| ![ULTIMATE JACKPOT](docs/screenshots/ultimate-jackpot.jpg) | |
| **The ULTIMATE** - five ULTIMATE symbols in a chain | |

*Screenshots are the core's own 1280x720 output, captured headlessly with
`tools/w7shot`; on the cabinet RetroArch adds a CRT-curve shader.*
...
| **GAMBLE** | X on any base-game win up to 50x | red/black doubles, a suit quadruples - an exactly fair draw |
...
| Button | Does |
|---|---|
| START / A | Spin; slam the reels down |
| LEFT / RIGHT | Bet less / more |
| X | Max bet and spin - or, on a win, **GAMBLE** |
| Y | Add credits |
| SELECT | Pay table (three pages) |
| B | Slam / cancel / collect |
...
(core-provided 16:9 aspect, the `zfast_crt_curve` shader if present) and a
`wild7.w7` settings file beside the launcher.
...
- **All art is procedural.** Every symbol, coin, card and wheel face is
  ...
  and shadowed, then blitted at run time. There are no image files.
